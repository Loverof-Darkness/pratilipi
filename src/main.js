...
const state = { dropId: null, uploadUrl: null, expiresAt: null, expiry: '1d', files: [], texts: [], busy: 0, view: 'home' };
...
function rememberDrop(data) {
  const items = history().filter((item) => item.id !== data.id);
  items.unshift({ id: data.id, uploadUrl: data.uploadUrl, expiresAt: data.expiresAt, expiry: data.expiry, createdAt: Date.now() });
  saveHistory(items);
}

function forgetDrop(id) {
...
function renderShell() {
  ...
  document.querySelector('#app').innerHTML = `
    <main class="app-shell">
      <header class="topbar">
        <a class="brand" href="/" aria-label="Pratilipi home"><span class="brand-devanagari">प्रतिलिपि</span><span class="brand-latin">Pratilipi</span></a>
        <nav class="top-nav" aria-label="Main navigation">
          <button id="nav-home" class="nav-btn active" type="button">Home</button>
          <button id="nav-active" class="nav-btn" type="button">Active Uploads</button>
        </nav>
        <button id="new-drop" class="ghost-btn" type="button">New Upload</button>
      </header>
      ...
      <section id="home-view">
        <section class="hero">
          <div>
            ...
            <p class="hero-copy">Upload files, paste text, scan a QR from your phone, and create direct download links.</p>
          </div>
          <div class="session-card">
            <div class="session-label">Current upload session</div>
            <div class="session-url-row"><input id="session-url" readonly aria-label="Upload session URL" /><button id="copy-session" class="icon-btn" type="button">Copy</button></div>
        ...
        <nav class="tabs" aria-label="Pratilipi sections"><button class="tab active" data-tab="files" type="button">Files & media</button><button class="tab" data-tab="text" type="button">Text paste</button></nav>

        <section id="panel-files" class="panel active-panel">
          <div id="drop-zone" class="drop-zone" tabindex="0"><input id="file-input" type="file" multiple hidden /><div class="drop-icon">＋</div><h2>Drop files here</h2><p>Drag & drop, paste from clipboard, or choose files directly.</p><button id="choose-files" class="primary-btn" type="button">Choose files</button><span class="helper">Images · songs · videos · PDFs · archives · any file type</span></div>
          <div id="upload-queue" class="queue"></div>
        </section>

        <section id="panel-text" class="panel">
          <div class="text-card"><textarea id="text-input" placeholder="Paste or type anything here…"></textarea><div class="text-toolbar"><span id="text-count">0 characters</span><button id="save-text" class="primary-btn" type="button">Save & create link</button></div></div>
      ...
      <section id="active-view" hidden>
        <section class="section-heading active-heading"><div><p class="eyebrow">LOCAL HISTORY</p><h1>Active uploads</h1><p class="hero-copy">Drops created on this browser/device. Each entry is checked with the server before it is shown.</p></div><button id="active-refresh" class="secondary-btn" type="button">Refresh</button></section>
    ...
    <dialog id="qr-dialog" class="qr-dialog"><div class="dialog-inner"><button id="close-qr" class="close-btn" aria-label="Close" type="button">×</button><p class="eyebrow">SCAN WITH PHONE</p><h3 id="qr-title">Pratilipi QR</h3><canvas id="qr-canvas"></canvas><input id="qr-url" readonly /><button id="copy-qr-url" class="secondary-btn" type="button">Copy URL</button></div></dialog>
  `;
}

function toast(message, kind = 'normal') {
  const el = $('#toast'); el.textContent = message; el.dataset.kind = kind; el.classList.add('show'); clearTimeout(toast.timer); toast.timer = setTimeout(() => el.classList.remove('show'), 2600);
}

function setBusy(delta) {
  state.busy = Math.max(0, state.busy + delta); document.body.classList.toggle('busy', state.busy > 0);
}
...
async function createDrop(expiry = $('#expiry-select')?.value || '1d') {
  ...
  $('#session-url').value = data.uploadUrl; $('#expiry-select').value = data.expiry;
...
async function loadDrop(dropId) {
  state.dropId = dropId; state.uploadUrl = `${location.origin}/u/${dropId}`; $('#session-url').value = state.uploadUrl;
  const data = await api(`/api/drop/${dropId}`);
  state.expiresAt = Number(data.drop.expires_at); state.expiry = data.drop.expiry_option || '1d'; state.files = data.files || []; state.texts = data.texts || [];
  rememberDrop({ id: dropId, uploadUrl: state.uploadUrl, expiresAt: state.expiresAt, expiry: state.expiry });
...
function renderResults() {
  ...
  document.querySelectorAll('[data-delete]').forEach((button) => button.onclick = () => deleteFile(button.dataset.delete));
}
...
async function uploadFile(file) {
  ...
  try {
    const form = new FormData(); form.append('file', file); form.append('upload_preset', CLOUDINARY_UPLOAD_PRESET); form.append('folder', 'pratilipi');
    ...
    bar.style.width = '80%';
    const done = await api(`/api/drop/${state.dropId}/complete`, { method: 'POST', body: JSON.stringify({ publicId: uploaded.public_id, secureUrl: uploaded.secure_url, resourceType: uploaded.resource_type, originalFilename: uploaded.original_filename, name: file.name, contentType: file.type || 'application/octet-stream', bytes: uploaded.bytes || file.size, format: uploaded.format || '' }) });
    bar.style.width = '100%'; item.classList.add('done'); state.files.push(done.file); renderResults(); toast(`${file.name} uploaded`);
...
async function saveText() {
  const textarea = $('#text-input'); const content = textarea.value; if (!content.trim()) return toast('Paste some text first.', 'error'); setBusy(1);
  try { await api(`/api/drop/${state.dropId}/text`, { method: 'POST', body: JSON.stringify({ content }) }); textarea.value = ''; updateTextCount(); await refreshDrop(); toast('Text saved'); }
...
async function refreshDrop() {
  ...
  try { const data = await api(`/api/drop/${state.dropId}`); state.files = data.files || []; state.texts = data.texts || []; state.expiresAt = Number(data.drop.expires_at); renderResults(); $('#session-status').textContent = `Active · ${formatRemaining(state.expiresAt)} · expires ${new Date(state.expiresAt).toLocaleString()}`; }
  catch (error) { if (error.message.includes('expired')) { forgetDrop(state.dropId); toast('This upload has expired.', 'error'); } }
}

async function deleteFile(fileId) {
  try { await api(`/api/drop/${state.dropId}/file/${fileId}`, { method: 'DELETE', body: JSON.stringify({}) }); state.files = state.files.filter((file) => file.id !== fileId); renderResults(); toast('File deleted'); }
  catch (error) { toast(error.message, 'error'); }
}
...
async function showActiveUploads() {
  ...
  for (const record of records) {
    try { const data = await api(`/api/drop/${record.id}`); valid.push({ ...record, ...data.drop, files: data.files?.length || 0, texts: data.texts?.length || 0 }); }
    catch { /* expired/deleted: do not keep */ }
  }
  saveHistory(valid.map((item) => ({ id: item.id, uploadUrl: `${location.origin}/u/${item.id}`, expiresAt: item.expires_at, expiry: item.expiry_option, createdAt: item.created_at })));
  list.innerHTML = valid.length ? valid.map((item) => `<article class="active-card"><div><strong>Upload ${esc(item.id.slice(0, 10))}…</strong><span>${item.files + item.texts} item${item.files + item.texts === 1 ? '' : 's'} · ${esc(EXPIRY_OPTIONS[item.expiry_option] || item.expiry_option)} · ${formatRemaining(item.expires_at)}</span></div><div class="active-actions"><button data-open="${esc(item.id)}" class="primary-btn" type="button">Open</button><button data-copy-active="${esc(item.uploadUrl || `${location.origin}/u/${item.id}`)}" class="mini-btn" type="button">Copy URL</button><button data-remove-active="${esc(item.id)}" class="mini-btn danger" type="button">Remove</button></div></article>`).join('') : `<div class="empty-state">No active uploads on this browser yet.</div>`;
  list.querySelectorAll('[data-open]').forEach((button) => button.onclick = () => openDrop(button.dataset.open));
  list.querySelectorAll('[data-copy-active]').forEach((button) => button.onclick = () => copyText(button.dataset.copyActive));
  list.querySelectorAll('[data-remove-active]').forEach((button) => button.onclick = () => { forgetDrop(button.dataset.removeActive); showActiveUploads(); });
}

async function openDrop(id) { state.view = 'home'; $('#home-view').hidden = false; $('#active-view').hidden = true; $('#nav-home').classList.add('active'); $('#nav-active').classList.remove('active'); history.replaceState({}, '', `/u/${id}`); try { await loadDrop(id); } catch (error) { toast(error.message, 'error'); } }

function setupEvents() {
  $('#nav-home').onclick = () => { state.view = 'home'; $('#home-view').hidden = false; $('#active-view').hidden = true; $('#nav-home').classList.add('active'); $('#nav-active').classList.remove('active'); };
  $('#nav-active').onclick = async () => { state.view = 'active'; $('#home-view').hidden = true; $('#active-view').hidden = false; $('#nav-home').classList.remove('active'); $('#nav-active').classList.add('active'); await showActiveUploads(); };
  $('#active-refresh').onclick = showActiveUploads;
  $('#new-drop').onclick = async () => { try { await createDrop($('#expiry-select')?.value || '1d'); state.files = []; state.texts = []; renderResults(); toast('New upload session created'); } catch (error) { toast(error.message, 'error'); } };
  $('#expiry-select').onchange = async () => { if (state.dropId) toast('Expiry is fixed when a Drop is created. Use New Upload to choose a different duration.'); };
...
async function boot() {
  renderShell(); setupEvents();
  const match = location.pathname.match(/\/u\/([^/]+)/);
  try {
    if (match) await loadDrop(match[1]); else await createDrop('1d');
  } catch (error) { $('#session-status').textContent = error.message; toast(error.message, 'error'); }
  state.poller = setInterval(refreshDrop, 20000);
  setInterval(() => { if (state.expiresAt && state.dropId) $('#session-status').textContent = `Active · ${formatRemaining(state.expiresAt)} · expires ${new Date(state.expiresAt).toLocaleString()}`; }, 30000);
}
